...
import com.tompkins_development.engine.lighting.*;
...
public class LightControls implements IGuiInstance {
    ...
    public LightControls(Scene scene) {
        ...
        if(sceneLights.getPointLights().size() != 0) {
            pointLight = sceneLights.getPointLights().get(0);
        ...
        if(sceneLights.getSpotLights().size() != 0) {
            spotLight = sceneLights.getSpotLights().get(0);
    ...
    public void drawGui() {
        ...
        if (ImGui.collapsingHeader("Dir Light")) {
            ImGui.sliderFloat("Dir Light - x", dirLightX, -10.0f, 10.0f, "%.2f");
            ImGui.sliderFloat("Dir Light - y", dirLightY, -10.0f, 10.0f, "%.2f");
            ImGui.sliderFloat("Dir Light - z", dirLightZ, -10.0f, 10.0f, "%.2f");
